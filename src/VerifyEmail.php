<?php
  ...
  class VerifyEmail {
    ...
    private $_aol_signup_page_url = 'https://login.aol.com/account/create?src=noSrc&specId=yidReg';
    private $_aol_signup_ajax_url = 'https://login.aol.com/account/module/create?validateField=yid';
    private $_aol_domains = array('aol.com');
    ...
    private $_protonmail_signup_page_url = 'https://mail.protonmail.com/create/new?language=en';
    private $_protonmail_username_check_url = 'https://mail.protonmail.com/api/users/available';
    private $_protonmail_domains = array('protonmail.com', 'protonmail.ch');
    private $_mail_ru_signup_page_url = 'https://account.mail.ru/signup?from=main&rf=auth.mail.ru';
    private $_mail_ru_username_check_url = 'https://account.mail.ru/api/v1/user/exists';
    private $_mail_ru_domains = array('mail.ru');
    ...
    public function set_proxy($host, $port, $username='', $pass='') {
      $this->debug[] = 'Setting proxy with '.$host.':'.$port;
    ...
    public function verify() {
      ...
      else if(in_array(strtolower($domain), $this->_aol_domains)) {
        $is_valid = $this->validate_aol();
      }
      ...
      else if(in_array(strtolower($domain), $this->_protonmail_domains)){
        $is_valid = $this->validate_protonmail();
      }
      else if(in_array(strtolower($domain), $this->_mail_ru_domains)){
        $is_valid = $this->validate_mail_ru();
      }
      //otherwise check the normal way
        ...
        if(preg_match("/^220/i", $out = fgets($this->connect))){
          ...
          while($line = fgets($this->connect, 128)){
              $to .= $line;
          }
          ...
          $this->debug[] = 'Looking for 550 (invalid) or 250 (valid) response...';
          if(preg_match('/^550/im', $to)) {
            $this->debug[] = '550 response found! Email is invalid.';
            $is_valid = false;
          }
          else if(!preg_match("/^250/i", $from) && !preg_match("/^250/i", $to)){
            $this->debug[] = '250 response Not found! Email is invalid.';
            $is_valid = false;
          }
          else{
    ...
    private function connect_mx() {
      ...
      stream_set_timeout($this->connect, $this->socket_timeout);
    }
    ...
    private function validate_aol() {
      $this->debug[] = 'Validating an aol email address...';
      $this->debug[] = 'Getting the sign up page content...';
      $this->fetch_page('aol');
      ...
      $this->debug[] = 'Adding the email to fields...';
      $fields['yid'] = str_replace('@aol.com', '', strtolower($this->email));

      $this->debug[] = 'Ready to submit the POST request to validate the email.';

      $response = $this->request_validation('aol', $cookies, $fields);
    ...
    private function validate_mail_com() {
      ...
      if(!$json_response['emailAddressAvailable']){
        $this->debug[] = 'Email address is not available, so it is valid.';
        return true;
      }
    ...
    private function validate_protonmail() {
      $this->debug[] = 'Validating a ProtonMail email address...';
      $this->debug[] = 'Getting the sign up page content...';
      $this->fetch_page('protonmail');

      $extra_headers = array();
      $this->debug[] = 'Setting the extra headers...';
      $extra_headers[] = 'x-pm-appversion: Web_3.15.34';
      $extra_headers[] = 'x-pm-apiversion: 3';
      
      $cookies = $this->get_cookies();

      $fields = array();
      $fields['Name'] = explode('@', $this->email)[0];

      $this->debug[] = 'Ready to submit the request to validate the email.';
      $response = $this->request_validation('protonmail', $cookies, $fields, $extra_headers);

      $this->debug[] = 'Searching availability code...';
      $json_response = json_decode($response, true);

      if($json_response['Code'] == '12106') {
        $this->debug[] = 'Email is not available, so it is valid.';
        return true;
      }
      $this->debug[] = 'Email is available, so it is not valid.';
      return false;
    }

    private function validate_mail_ru() {
      $this->debug[] = 'Validating a Mail.ru email address...';
      $this->debug[] = 'Checking with username check URL...';
      $fields = array();
      $fields['email'] = $this->email;

      $this->debug[] = 'Ready to submit the request to validate the email.';
      $response = $this->request_validation('mail_ru', array(), $fields);

      $this->debug[] = 'Searching availability code...';
      $json_response = json_decode($response, true);

      if($json_response['body']['exists']) {
        $this->debug[] = 'Email exists, so it is valid.';
        return true;
      }
      $this->debug[] = 'Email is available, so it is not valid.';
      return false;
    ...
    private function fetch_page($service, $cookies = ''){
      ...
      if($service == 'yahoo'){
        $url = $this->_yahoo_signup_page_url;
      }
      else if($service == 'aol'){
        $url = $this->_aol_signup_page_url;
      }
      else if($service == 'hotmail'){
        $url = $this->_hotmail_signin_page_url;
      }
      else if($service == 'mail'){
        $url = $this->_mail_com_signup_page_url;
      }
      else if($service == 'protonmail'){
        $url = $this->_protonmail_signup_page_url;
      }
      ...
      if($this->page_content === false){
        ...
        $this->page_headers = $response_headers;
    ...
    private function request_validation($service, $cookies, $fields, $extra_headers=null){
      if($service == 'yahoo'){
        ...
        $headers = array();
        $headers[] = 'Origin: https://login.aol.com';
        $headers[] = 'X-Requested-With: XMLHttpRequest';
        $headers[] = 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.71 Safari/537.36';
        $headers[] = 'content-type: application/x-www-form-urlencoded; charset=UTF-8';
        $headers[] = 'Accept: */*';
        $headers[] = 'Referer: https://login.aol.com/account/create?src=noSrc&specId=yidReg';
        $headers[] = 'Accept-Encoding: gzip, deflate, br';
        $headers[] = 'Accept-Language: en-US,en;q=0.8,ar;q=0.6';
        ...
        $headers[] = 'Referer: https://login.live.com/login.srf?wa=wsignin1.0';
        $headers[] = 'Accept-Encoding: gzip, deflate, br';
        $headers[] = 'Accept-Language: en-US,en;q=0.8,ar;q=0.6';
        ...
        $url = $this->_hotmail_username_check_url;
      } else if($service == 'mail'){ 
        $headers = array();
        ...
        $headers[] = 'Cookie: '.$cookies_str;
        $postdata = json_encode($fields);
        $url = $this->_mail_com_username_check_url;
      } else if($service == 'protonmail'){ 
        $headers = array();
        $headers[] = 'Origin: https://protonmail.com';
        $headers[] = 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36';
        $headers[] = 'Content-type: application/json; charset=UTF-8';
        $headers[] = 'Accept: application/json, text/plain, */*';
        $headers[] = 'Referer: https://mail.protonmail.com/create/new?language=en';
        $headers[] = 'Accept-Encoding: gzip, deflate, br';
        $headers[] = 'Accept-Language: en-US,en;q=0.9';
        ...
        $postdata = array();
        $url = $this->_protonmail_username_check_url . '?' . $params;
      }
      else if($service == 'mail_ru'){
        $headers = array();
        $headers[] = 'authority: account.mail.ru';
        ...
        $headers[] = 'origin: https://account.mail.ru';
        $headers[] = 'sec-fetch-site: same-origin';
        $headers[] = 'sec-fetch-mode: cors';
        $headers[] = 'referer: https://account.mail.ru/signup?from=main&rf=auth.mail.ru';
        $headers[] = 'accept-language: en-US,en;q=0.9';
        ...
        $url = $this->_mail_ru_username_check_url . '?' . $params;
      }
